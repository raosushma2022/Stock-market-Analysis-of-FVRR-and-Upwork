...
data= pd.read_csv("UPWK.csv",index_col='Date',parse_dates=True)
...
# From the histogram, we can observe the price change percentage mostly fall between the range of -0.2% to 0.2%.
...
predictions.to_csv("Predicted-price-data1.csv")
#colllects future days from predicted values
oneyear_df = pd.DataFrame(predictions[:252])
oneyear_df.to_csv("one-year-predictions1.csv")
onemonth_df = pd.DataFrame(predictions[:21])
onemonth_df.to_csv("one-month-predictions1.csv")
fivedays_df = pd.DataFrame(predictions[:5])
fivedays_df.to_csv("five-days-predictions1.csv")
...
##Loading the Excel files 
df1= pd.read_csv("UPWK.csv",index_col='Date',parse_dates=True)
...
plt.plot(day_new,scaler.inverse_transform(df1[120:]))
...
# CONCLUSION 
# 
# The stock price of UPWK  is expected to slightly increase or stay flat in the coming months
